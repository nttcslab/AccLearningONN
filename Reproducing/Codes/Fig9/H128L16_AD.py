...
parser.add_argument('--num_finelayers',type=int,default=16)
parser.add_argument('--num_layers',type=int,default=8)
parser.add_argument('--epochs',type=int,default=20)
parser.add_argument('--seed',type=int,default=202104162)
parser.add_argument('--outfile',type=str,default='../../Results/Fig9/H128L16_AD.log')
...
''' Diagonal unitary matrix linear function'''
...
def forwardPSDC(inX, inY, angle):
	_,batch_size = inX.shape
	invSqrt2 = 1.0/np.sqrt(2.0)
	cosP, sinP = torch.cos(angle), torch.sin(angle)
	cosP_bcast = cosP.repeat(1,batch_size)
	sinP_bcast = sinP.repeat(1,batch_size)
	val1 = cosP_bcast*inX.real -sinP_bcast*inX.imag
	val2 = sinP_bcast*inX.real +cosP_bcast*inX.imag
	outXR = val1 -inY.imag
	outXI = val2 +inY.real
	outYR = -val2 +inY.real
	outYI = val1 +inY.imag
	outX = invSqrt2*torch.complex(outXR,outXI)
	outY = invSqrt2*torch.complex(outYR,outYI)
	return outX, outY
...
class MZIA(nn.Module):
	...
	def forward(self, cmplx):
		...
		outX, outY = forwardPSDC(cmplx[0:-1:2,:],cmplx[1::2,:],self.angleA0)
		output[0:-1:2,:], output[1::2,:] = forwardPSDC(outX,outY,self.angleA1)
		return output
...
class MZIB(nn.Module):
	...
	def forward(self, cmplx):
		...
		outX, outY = forwardPSDC(cmplx[1:-1:2,:],cmplx[2::2,:],self.angleB0)
		output[1:-1:2,:], output[2::2,:] = forwardPSDC(outX,outY,self.angleB1)
		return output
